...
def retrieval(result):
    ...
    if (result):
        for index in result:
            ...
            documents[doc_makanan_nama] = doc_makanan_bahan
    else:
        documents = {}
...
input_1 = 'ayam and ikan'

result = process_query(input_1)

get_doc = retrieval(result)
# print(get_doc)
A = len(get_doc)
B = 0
C = 0
D = len(splitted_docs_makanan_with_bahan) - len(get_doc)
print(f"\nAll documents {len(splitted_docs_makanan_with_bahan)}"
      f"\nDokumen retrived {A}")
print(f"\nRetrived Relevan {A}"
      f"\nRetrived Not Relevan {B}"
      f"\nNot Retrived Relevan {C}"
      f"\nNot Retrived Not Relevan {D}")
